...
Sys.time()
format(Sys.time(), "%d_%b_%Y_%H.%M.%S")

write_json(data1,paste0("EG0755", format(Sys.time(), "%d_%b_%Y_%H.%M.%S"), ".json"))

paste0("EG0755", format(Sys.time(), "%d_%b_%Y_%H.%M.%S"), ".json")
...
write.csv(output, paste0("MensRunning", format(Sys.time(), "%d_%b_%Y_%H.%M.%S"), ".csv"))
write.csv(allpagesdf, file = "MensRunning.csv")
